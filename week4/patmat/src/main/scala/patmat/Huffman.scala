...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: trees_ => (new Fork(left, right, chars(left) ::: chars(right), weight(left) + weight(right)) :: trees_).sortWith(weight(_) < weight(_))
    case _ => trees
  }
  /**
   ...
   */
  def until[A](char: A => Boolean, f: A => A)(xs: A): A =
    if (char(xs)) xs else until(char, f)(f(xs))
