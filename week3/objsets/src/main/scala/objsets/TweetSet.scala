...
abstract class TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet =
    {
      def predicateSet(p: Tweet => Boolean, t: Tweet) = {
        //      print("call predicate " + t)
        if (p(t)) {
          //        println(" is true")
          new NonEmpty(t)
        } else {
      ...
      if (accu.isEmpty) {
        ...
        predicateSet(p, accu.head) union filter0(p, accu.tail)
      }
    }
  ...
  def ascendingByRetweet: Trending = {
    ...
    def ascendingByRetweet(accu: TweetSet): Trending = {
//      println(size(accu))
      if (accu.isEmpty) new EmptyTrending
      else {
        val t = accu.findMin
        
        new NonEmptyTrending(t, ascendingByRetweet(accu.remove(t)))
      }
    }
    ascendingByRetweet(this)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  //    }
  //    accu match {
  //      case e: Empty => new Empty
  //      case n: NonEmpty => predicateSet(p, elem) union filter0(p, n.l) union filter0(p, n.r)
  //    }
  //
  //  }
...
object GoogleVsApple {
  ...
  def containsGoogle(t: Tweet): Boolean = {
    google.exists(t.text.contains(_))
  }
  
  def containsApple(t: Tweet): Boolean = {
    apple.exists(t.text.contains(_))
  }

  
  val googleTweets: TweetSet = TweetReader.allTweets.filter(containsGoogle)

  val appleTweets: TweetSet = TweetReader.allTweets.filter(containsApple)

  // Q: from both sets, what is the tweet with highest #retweets?
  val trending: Trending = TweetReader.allTweets.ascendingByRetweet
}
